...
{	
	...
	private String userMacAddr;//用户地址
	...
	/**
	* @Title: setLoginService 
	* @Description: 注入loginService
	* @param loginService    参数 
	* @return void    返回类型 
	* @throws 
	 */
	@Autowired
	...
	{	
		...
		CaptchaUsernamePasswordToken token=new CaptchaUsernamePasswordToken();//创建自定义验证码用户密码令牌
        token.setUsername(userName);
        token.setPassword(password.toCharArray());
        token.setCaptcha(captcha);
        token.setRememberMe(true);//?系统默认记住密码?
        Json json=new Json();
        json.setTitle("登录提示");
        try{
            subject.login(token);
            ...
            subject.login(token);
            logger.error(Constants.UNKNOWN_SESSION_EXCEPTION);//会话异常
            json.setMessage(Constants.UNKNOWN_SESSION_EXCEPTION);
        }
        catch(UnknownAccountException ex){
			logger.error(Constants.UNKNOWN_ACCOUNT_EXCEPTION);//账号错误
			json.setMessage(Constants.UNKNOWN_ACCOUNT_EXCEPTION);
		}
        catch (IncorrectCredentialsException ice) {
            json.setMessage(Constants.INCORRECT_CREDENTIALS_EXCEPTION);//密码错误
        } 
        catch (LockedAccountException lae) {
            json.setMessage(Constants.LOCKED_ACCOUNT_EXCEPTION);//账号锁定
        }catch (IncorrectCaptchaException e) {
        	 json.setMessage(Constants.INCORRECT_CAPTCHA_EXCEPTION);//验证码错误
		}
        catch (AuthenticationException ae) {
            json.setMessage(Constants.AUTHENTICATION_EXCEPTION);//没有授权
        } 
        catch(Exception e){
            json.setMessage(Constants.UNKNOWN_EXCEPTION);//未知异常
        }
