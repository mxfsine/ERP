...
public class PermissionAssignmentServiceImpl implements PermissionAssignmentService
{
	...
	public List<Permission> getRolePermission(Integer roleId)
	{
		...
		List list = publicDao.findBySQL(sql);//查询权限ID
	...
	{
		Integer userId = Constants.getCurrendUser().getUserId();//得到用户ID
		Role role = this.getRole(roleId);//通过角色ID查询到角色
		...
		{
			Integer permissionId = rolePermission.getPermission().getPermissionId();//得到角色所拥有的权限ID
	...
	*/
	...
	public boolean persistenceRole(Integer roleId) {
		...
		role.setStatus(Constants.PERSISTENCE_DELETE_STATUS);//假删除，设置状态为I
		publicDao.deleteToUpdate(role);
		return true;
	}
