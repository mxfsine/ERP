		...
		centerTabs = $('#centerTabs').tabs({
						tools : [ {
							//选项卡页面刷新
								iconCls : 'icon-reload',
								handler : function() {
									var href = $('#centerTabs').tabs('getSelected').panel('options').href;//获取选择的选项卡面板→获取面板属性对象→获取herf
									if (href) {/*说明tab是以href方式引入的目标页面*/
										var index = $('#centerTabs').tabs('getTabIndex', $('#centerTabs').tabs('getSelected'));//获取当前tab索引
										$('#centerTabs').tabs('getTab', index).panel('refresh');//刷新当前panel
									} else {   /*说明tab是以content方式引入的目标页面*/
										var panel = $('#centerTabs').tabs('getSelected').panel('panel');//获取当前选项卡面板对象
										var frame = panel.find('iframe');//获取所有iframe元素
										try {
											if (frame.length > 0) {
